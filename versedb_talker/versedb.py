...
class VerseDBTalker(ComicTalker):
    ...
    about: str = (
        ...
        f"manhua, bande dessinée and magazines, with a REST API for cataloguing and discovery."
    ...
    def __init__(self, version: str, cache_folder: pathlib.Path) -> None:
        ...
        # Default settings
        self.default_api_url = self.api_url = f"{self.website}/api/v1/"
    ...
    def _map_issue_detail(self, issue: dict[str, Any]) -> GenericMetadata:
        ...
        md.locations = self._split_locations(issue.get("locations"))
    ...
    @staticmethod
    def _split_locations(raw: Any) -> set[str]:
        if not isinstance(raw, str):
            return set()
        # locations is free text; a semicolon, when present, separates entries that
        # themselves contain commas ("Colville, Missouri; Daniel's Truck Stop")
        delimiter = ";" if ";" in raw else ","
        return {loc.strip() for loc in raw.split(delimiter) if loc.strip()}
    ...
    def _site_url(self) -> str:
        # api_url is the versioned /api/v1 path (or the bare /api alias); web links live at the site root
        return self.api_url.rstrip("/").removesuffix("/v1").removesuffix("/api")
